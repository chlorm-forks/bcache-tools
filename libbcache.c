...
void do_pwrite(int fd, const void *buf, size_t bytes, u64 offset)
{
	ssize_t ret;

	while (bytes) {
		ret = pwrite(fd, buf, bytes, offset);
		if (ret < 0)
		   ...
		   uuid_le uuid)
{
	...
	if (passphrase) {
		...
		bcache_crypt_init(sb, crypt, passphrase);
	...
	for (i = devs; i < devs + nr_devs; i++) {
		...
		/* Zero start of disk */
		do_pwrite(i->fd, zeroes, BCH_SB_SECTOR << 9, 0);

		do_pwrite(i->fd, &sb->layout, sizeof(sb->layout),
			  BCH_SB_LAYOUT_SECTOR << 9);

		for (j = 0; j < sb->layout.nr_superblocks; j++) {
			sb->offset = sb->layout.sb_offset[j];

			sb->csum.lo = csum_vstruct(BCH_SB_CSUM_TYPE(sb), sb);
			do_pwrite(i->fd, sb, vstruct_bytes(sb),
...
struct bch_sb *bcache_super_read(const char *path)
{
	...
	if (pread(fd, &sb, sizeof(sb), BCH_SB_SECTOR << 9) != sizeof(sb))
	...
	if (pread(fd, ret, bytes, BCH_SB_SECTOR << 9) != bytes)
		die("error reading superblock");
...
void bcache_super_print(struct bch_sb *sb, int units)
{
	...
	printf("External UUID:			%s\n"
	       ...
	       pr_units(BCH_SB_BTREE_NODE_SIZE(sb), units).b,

	       BCH_SB_ERROR_ACTION(sb) < BCH_NR_ERROR_ACTIONS
	       ? error_actions[BCH_SB_ERROR_ACTION(sb)]
	       : "unknown",
	       ...
	       ? csum_types[BCH_SB_META_CSUM_TYPE(sb)]
	       : "unknown",

	       BCH_SB_DATA_CSUM_TYPE(sb) < BCH_CSUM_NR
	       ? csum_types[BCH_SB_DATA_CSUM_TYPE(sb)]
	       : "unknown",

	       BCH_SB_COMPRESSION_TYPE(sb) < BCH_COMPRESSION_NR
	       ? compression_types[BCH_SB_COMPRESSION_TYPE(sb)]
	       : "unknown",

	       BCH_SB_STR_HASH_TYPE(sb) < BCH_STR_HASH_NR
	       ? str_hash_types[BCH_SB_STR_HASH_TYPE(sb)]
	       : "unknown",
	...
	mi = (void *) bch_sb_field_get(sb, BCH_SB_FIELD_members);
	if (!mi) {
	...
	for (i = 0; i < sb->nr_devices; i++) {
		...
		printf("\n"
		       ...
		       ? member_states[BCH_MEMBER_STATE(m)]
		       : "unknown",
		       ...
		       ? replacement_policies[BCH_MEMBER_REPLACEMENT(m)]
		       : "unknown",
